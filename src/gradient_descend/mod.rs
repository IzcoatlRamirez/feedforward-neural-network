...
use crate::numrs::math::{hadamard, lineal_transform, outer, tranpose,round_to_n_decimals};
...
fn subtrac_weight(l: &mut Layer, gradient: Vec<Vec<f64>>, learning_rate: f64) {
    for i in 0..l.rows {
        for j in 0..l.cols {
            l.weights[i as usize][j as usize] -= round_to_n_decimals(learning_rate * gradient[i as usize][j as usize]);
        }
    }
}
...
fn rounded_weights(l: &mut Layer){
    for i in 0..l.rows {
        for j in 0..l.cols {
            l.weights[i as usize][j as usize] = round_to_n_decimals(l.weights[i as usize][j as usize]);
        }
    }
}
